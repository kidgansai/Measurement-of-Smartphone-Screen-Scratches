...
void img_rotate(Mat & imgIn, Mat &imgOut, Point cent, double dAngle, int fillVal)
{
    ...
    cv::warpAffine(imgIn, imgOut, M, imgIn.size(), INTER_LINEAR + WARP_FILL_OUTLIERS, 0, cv::Scalar(fillVal));
}
...
vector<Point> drawRotRect1(Mat & img, Point p1, Point p2, Point p3)
{
	...
	int thinkness = 1;
        ...
        line(img, p1, p2, Scalar(0, 255, 0),thinkness);
        line(img, p3, p2, Scalar(0, 255, 0),thinkness);
        line(img, p3, p4, Scalar(0, 255, 0),thinkness);
        line(img, p1, p4, Scalar(0, 255, 0),thinkness);
...
outInfo   drawCVRotRect(Mat & img, Point p1, Point p2, Point p3,int thickness) //angle为0-180
{
    outInfo info;
        ...
        if (true)
        {
                ...
                //for (int i = 0; i < 4; i++)
                //{
                //    line(img, pts[i], pts[(i + 1) % 4], Scalar(0, 0,250), thickness);
                //}
        }
        info.angle = angle;
...
void showIntersection(string txtFile,Mat & img)
{
    ...
    cvtColor(dst, binDst, COLOR_BGR2GRAY);
